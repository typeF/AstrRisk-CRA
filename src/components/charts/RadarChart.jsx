...
class RadarChart extends Component {
  ...
  render() {
    ...
    const data = [
      [
        ...
        { axis: "Mass", value: valueParser(massScale(mass)) },
        {
          ...
          value: valueParser(velocityScale(speed * 1000)),
        },
    ...
    const cx = (value, i) => {
      return rScale(value) * Math.cos(angleSlice * i - Math.PI / 2);
    };
    const cy = (value, i) => {
      return rScale(value) * Math.sin(angleSlice * i - Math.PI / 2);
    };
    const lx = (value, i) => {
      return rScale(value) * Math.cos(angleSlice * i * 2 - Math.PI / 2);
    };
    const ly = (value, i) => {
      return rScale(value) * Math.sin(angleSlice * i * 2 - Math.PI / 2);
    };
    ...
    const dataDots = data[0].map((item, i) => (
      <circle
        cx={cx(item.value, i)}
        cy={cy(item.value, i)}
        r={4}
    ...
    const graphGrid = [1, 2, 3].map((item, i) => (
      <circle
        cx={0}
        cy={0}
        r={(radius * (i + 1)) / 3}
    ...
    // Generates axis path
    const graphAxis = [1, 2, 3, 4, 5].map((item, i) =>
      axisLines([
        { x: 0, y: 0 },
        { x: lx(1, i), y: ly(1, i) },
      ])
        ...
        <div className="radar-chart">
          ...
          >
            ...
            >
              ...
              <g
                id="radar-chart-data-blob"
                onClick={(e) => this.props.toggleDangerBar()}
                onMouseEnter={(e) => this.setState({ showDangerPrompt: true })}
                onMouseLeave={(e) => this.setState({ showDangerPrompt: false })}
              >
                <path key={267229} d={radarDataPath} />
              </g>
              <path
                id={"radar-blob-outline"}
                key={299933}
                stroke={"#4fd4fd"}
                d={radarDataPath}
                strokeWidth={"3px"}
          ...
          <div className="radar-chart-toggle-danger-bar">
            {this.state.showDangerPrompt && (
              <span>Click to show danger level</span>
            )}
          </div>
        </div>
      </Fragment>
